...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if ((*(__IO u16 *) (LcdDataAddr)))r = 0;							//dummy Read	   
	opt_delay(2);	  
	r = (*(__IO u16 *) (LcdDataAddr));  		  						//实际坐标颜色
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510)		//9341/NT35310/NT35510要分2次读出
 	{
		opt_delay(2);	  
		b = (*(__IO u16 *) (LcdDataAddr));
		g=r&0XFF;		//对于9341/5310/5510,第一次读取的是RG的值,R在前,G在后,各占8位
		g<<=8;
	}else if(lcddev.id==0X6804)r=(*(__IO u16 *) (LcdDataAddr));		//6804第二次读取的才是真实值 
...
void LCD_DrawPoint(u16 x,u16 y)
{
	...
	(*(__IO u16 *) (LcdRegAddr)) = POINT_COLOR;
}
//快速画点
//x,y:坐标
//color:颜色
...
{	   
	...
	(*(__IO u16 *) (LcdRegAddr)) = lcddev.wramcmd;
	(*(__IO u16 *) (LcdDataAddr)) = color;
}	 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for (j = 0; j<width; j++)(*(__IO u16 *) (LcdDataAddr)) = color[i*height + j];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
